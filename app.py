...
# Custom CSS — dark sci-fi theme with teal accents
st.markdown("""
<style>
    ...
    /* Buttons */
    .stButton > button {
        background: linear-gradient(135deg, #00e5c0, #0080ff) !important;
        color: #000 !important;
    ...
    /* Spinner */
    .stSpinner > div {
        border-top-color: #00e5c0 !important;
    }
</style>
...
try:
    ...
    selected_country = st.sidebar.selectbox("Country", countries, index=countries.index("India") if "India" in countries else 0)
    ...
    # ==============================
    # COUNTRY HISTORICAL OVERVIEW (always visible)
    # ==============================
    ...
    pop_change_pct = ((last['TotalPopulation'] - first['TotalPopulation']) / first['TotalPopulation']) * 100
    ...
    with hc1:
        ...
        fig_hist_pop.update_layout(**PLOT_LAYOUT, title=dict(text="Population Over Time", font=dict(color="#e0e8f0", size=13)), height=260)
    ...
    with hc2:
        ...
        fig_rates.update_layout(**PLOT_LAYOUT, title=dict(text="Birth & Death Rates per 1,000", font=dict(color="#e0e8f0", size=13)), height=260,
    ...
    if run_forecast:
        ...
        st.markdown(f"<div class='section-label'>Forecast Results — {selected_country} · {selected_year}</div>", unsafe_allow_html=True)
        ...
        with c1:
            st.metric("Total Population", f"{pred_pop:,}", delta_pop)
        with c2:
            st.metric("Male", f"{int(result['Male Population']):,}")
        with c3:
            st.metric("Female", f"{int(result['Female Population']):,}")
        with c4:
            st.metric("Urban", f"{int(result['Urban Population']):,}")
        with c5:
            st.metric("Rural", f"{int(result['Rural Population']):,}")

        r1, r2 = st.columns(2)
        with r1:
            st.metric("Birth Rate", f"{result['Birth Rate']:.2f} per 1,000")
        with r2:
            st.metric("Death Rate", f"{result['Death Rate']:.2f} per 1,000")
        ...
        fig_proj.add_trace(go.Scatter(
            ...
            name="±5% Uncertainty", showlegend=True
        ))
        # Vertical divider
        fig_proj.add_vline(x=last_hist_year, line_dash="dash", line_color="#1e3050", annotation_text="Today", annotation_font_color="#7fa8c9")
        ...
        with p2:
            # Projected Birth vs Death rate over time
        ...
        def score_badge(label, r2):
            score = max(0, r2)
            cls = "score-good" if score >= 0.95 else ("score-ok" if score >= 0.85 else "score-poor")
            icon = "✓" if score >= 0.95 else ("~" if score >= 0.85 else "✗")
            return f"<span class='score-badge {cls}'>{icon} {label}: R² {score:.3f}</span>"
        ...
        # Gender donut & urban donut
        st.markdown("<div class='section-label'>Forecast Breakdown — {}</div>".format(selected_year), unsafe_allow_html=True)
