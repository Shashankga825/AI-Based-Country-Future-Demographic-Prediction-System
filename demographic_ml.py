...
def train_models (country_df :pd .DataFrame )->TrainedModels :
    ...
    def _tune_ridge_pipeline (X :np .ndarray ,y :np .ndarray ,feature_names :List [str ])->tuple [object ,float ,float ,float ]:
        ...
        Uses a time-based holdout for evaluation so the reported score always comes from
        unseen data. Very small series fall back to a minimal holdout or a neutral score.
...
end_year :int ,
    ...
    if len (country_df )>2 :
        current_growth_rate =float (country_df ["TotalPopulation"].pct_change ().dropna ().iloc [-1 ])
    else :
        current_growth_rate =historical_growth_rate 
